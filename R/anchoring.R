...
#'   \code{range_input}. Then, this number is hashed by calling the
...
#' @param range_input A number giving the range from which to draw
#'   a random number.
...
#' @param nchar_output A number indicating how long the hash sequene should be.
#'   \code{NULL} indicates no limitation.
#' @keywords internal
get_anchor <- function(enclosing_start = "#< ",
  ...
  random_nr <- sample(range_input, 1)
  hash <- digest(random_nr)

  # shorten output if desired
  if (!is.null(nchar_output)) {
    hash <- substring(hash, 1, nchar_output)
  }

  # return anchor
  paste0(enclosing_start, hash, enclosing_end)
